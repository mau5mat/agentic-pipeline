...
Subagents spawned by the orchestrator start blank — no CLAUDE.md, no memory files. Fixed in `pipeline.md` in two passes: (1) the orchestrator reads all `feedback_*.md` files from both repo-specific and Slice-level memory directories and injects them as hard constraints into every subagent prompt; (2) extended to also read and inject `CLAUDE.md`/`AGENTS.md` from the repo root, and the `### Repo style` section from the WorkItem. The orchestrator now reports what was loaded after Step 1 — missing or empty sources are surfaced, not silently skipped.
...
- **#6 — WorkItem directory.** WorkItems moved from `pr-descriptions/` to `workitems/`. All skill files and docs updated. Handover docs and PR bodies remain in `pr-descriptions/` — separation is intentional.
